...
```{r}
glm.model2 <-glm(Time ~ Distance + AccumIncr, data=data, family=gaussian(link = "log"))
...
```{r}
glm.model3 <-glm(Time ~ Distance + AccumIncr, data=data, family=gaussian(link = "inverse"))
...
So we are going to stick to our Linear model as our predicting model, for now.
