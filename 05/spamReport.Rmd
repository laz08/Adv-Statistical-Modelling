...
```{r}
sc.train.set[, 58] <- as.numeric(sc.train.set[, 58])
logit.lasso = cv.glmnet(as.matrix(sc.train.set[, 1:57]), as.factor(sc.train.set[, 58]), alpha=1, family="binomial", standardize = FALSE)

predict(logit.lasso, as.matrix(sc.val.set), type="response")
#y.pred2 <- predict.glmnet(x=logit.lasso, y=as.matrix(sc.val.set), s=c(logit.lasso$lambda.min), type="response")
head(y.pred2); summary(y.pred2)

plot(logit.lasso,  xvar="lambda")
```
