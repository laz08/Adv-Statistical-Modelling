...
It seems that in this case, we are selecting the one with $\lambda_{9} = 45.41$.
...
It is clear how the method of validation is the one that gives the less error for the values of parameters in the effective degrees of freedom. Yet the number of parameters established by the degrees of freedom is different for the validation ridge regression than for the other. In the validation approach the optimal amount of df is __5.88__ while for the other methods it seems that the optimal amount of parameters is __6.79__
...
the MSPE 5-fold evolution and in fact have a greater value when df.v > 9. Yet the different methods provide different optimal number for optimal parameters. For all methods besides validation we can see how the optimal number of parameters is 13, while for the validation is 7.